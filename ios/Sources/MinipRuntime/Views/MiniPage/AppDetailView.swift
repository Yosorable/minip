    ...
    init(appInfo: AppInfo, parentVC: UIViewController?, detailVC: AppDetailViewController?) {
        ...
        if let icon = appInfo.icon {
            if icon.starts(with: "http://") || icon.starts(with: "https://") {
                ...
                self.iconURL = Global.shared.miniAppsRootURL.appending(path: self.appInfo.name, directoryHint: .isDirectory).appending(path: icon)
            }
        }
    }
        ...
        Button {
            ...
            VStack {
                Image(systemName: "gear").font(.system(size: 30))
                    ...
                    .background(Color("BlockButtonBackground", bundle: .module))
